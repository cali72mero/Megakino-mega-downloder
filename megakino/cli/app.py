...
from megakino.api.scraper import get_media_details, search_for_movie
...
from megakino.core.config import CONFIG_FILE, config, config_manager
from megakino.core.streams import resolve_episode_streams
...
def show_uninstall_guide():
    ...
    console.print(
        "[yellow]So entfernst du das Programm und alle Hilfsprogramme vollständig:[/yellow]"
    )
...
async def settings_menu():
    while True:
        ...
        if choice == "Back":
            ...
            new_path = await inquirer.text(
                message="Enter new download path:", default=config.download_path
            ).execute_async()
            ...
            console.print(
                "Es geht nur um das Aussehen des Programms (Lade-Animationen und Farben im Terminal)."
            )
            console.print(
                " - [cyan]AN[/cyan]: Wenn du einen schnellen PC hast und es schick aussehen soll."
            )
            console.print(
                " - [cyan]AUS[/cyan]: Wenn das Terminal flackert oder du es lieber ganz schlicht magst.\n"
            )

            config.show_animations = await inquirer.confirm(
                message="Menü-Effekte / Animationen einschalten?", default=config.show_animations
            ).execute_async()
...
async def search_flow(client: APIClient):
    ...
    selected_url = await inquirer.select(
        message="Select a result:", choices=choices
    ).execute_async()
    ...
    with console.status("[bold green]Fetching details...[/bold green]"):
        try:
            ...
            console.print(f"[red]Fetching details failed: {e}[/red]")
            return
    ...
    provider = await inquirer.select(
        message="Select provider:", choices=["Megakino", "VOE"], default=config.preferred_provider
    ).execute_async()

    with console.status("[bold green]Suche nach funktionierenden Streams...[/bold green]"):
        resolution = await resolve_episode_streams(selected_episodes, client, provider)

    for failed in resolution.failed:
        console.print(
            f"[bold red]Fehler: Kein funktionierender Link für '{failed.episode.title}' "
            f"bei {failed.primary_provider} oder {failed.fallback_provider} gefunden![/bold red]"
        )

    direct_links = [item.direct_link for item in resolution.resolved]
    final_episodes = [item.episode for item in resolution.resolved]
